...
function buildMembershipGateHtml(reqUrl){
  const u = (reqUrl && reqUrl.href) ? reqUrl : { origin:'', pathname:'', search:'' };
  const origin = u.origin || '';
  const next = (u.pathname || '') + (u.search || '');
  const sampleUrl = origin + '/sample/';
  const subscribeUrl = origin + '/subscribe/';
  const homeUrl = origin + '/';
  const loginUrl = origin + '/account/?next=' + encodeURIComponent(next);
...
<style>
  :root{
    ...
    --blue:#2563eb;
    --blue2:#38bdf8;
    --grad:linear-gradient(90deg,var(--blue),var(--blue2));
    --radius:18px;
  }
  *{box-sizing:border-box}
  body{
    ...
    color:var(--txt);
    background:
      radial-gradient(900px 520px at 18% 0%, rgba(37,99,235,.14), transparent 55%),
      radial-gradient(900px 520px at 88% 22%, rgba(56,189,248,.13), transparent 55%),
  ...
  .topline{height:5px;background:var(--grad)}
  .head{
  ...
  .card{
    ...
    background:linear-gradient(180deg, rgba(37,99,235,.03), rgba(56,189,248,.02));
    border-radius: var(--radius);
    padding:16px 16px 14px;
  }
  .card h3{margin:0 0 10px;font-size:15px;letter-spacing:-.1px}
  ...
  .mini{margin-top:12px;color:rgba(15,23,42,.62);font-size:12.5px;line-height:1.65}
  .cta{
    border:1px solid rgba(15,23,42,.08);
    background:#fff;
    border-radius: var(--radius);
  ...
  .k{font-weight:900;color:rgba(15,23,42,.85)}
</style>
</head>
<body>
  <div class="wrap">
    <div class="panel">
      ...
      <div class="head">
        <div>
          <div class="badge">빅데이터센터 · 구독 멤버십 전용</div>
          <div class="title">이 공간은 구독 멤버십 전용입니다</div>
          <p class="desc">비구독자는 빅데이터센터(강한/매집/수상해)를 열람할 수 없습니다. 대신 <b>샘플자료</b>로 품질을 먼저 확인할 수 있습니다.</p>
        </div>
        <div class="badge">요청 경로: <span class="k">${next.replace(/</g,'&lt;')}</span></div>
      </div>

      <div class="grid">
        <div class="card">
          <h3>이용 방식</h3>
          <p>로그인 가능한 계정은 <b>구독자</b>만 보유합니다. 비구독자는 로그인할 수 없으며, 사이트의 다른 메뉴는 계속 이용할 수 있습니다.</p>
          <ul class="list">
            <li class="li"><span class="dot"></span><span><b>구독자</b>: 빅데이터센터 전체 열람</span></li>
            <li class="li"><span class="dot"></span><span><b>비구독자</b>: 샘플자료실/홈의 샘플로 미리보기</span></li>
            <li class="li"><span class="dot"></span><span><b>샘플자료</b>: 무료 공개 (홈/샘플자료실)</span></li>
          </ul>
          <div class="mini">샘플을 먼저 확인하신 뒤, 필요하면 구독 안내를 통해 가입 후 로그인하시면 됩니다.</div>
          <div class="row" style="margin-top:12px;">
            <span class="pill">홈은 공개</span>
            <span class="pill">공지/뉴스/성과 공개</span>
            <span class="pill">빅데이터만 잠금</span>
          </div>
        </div>

        <div class="cta">
          <a class="btnPrimary btn" href="${sampleUrl}">샘플자료실 보기</a>
          <a class="btn" href="${homeUrl}">홈으로</a>
          <a class="btn" href="${subscribeUrl}">구독 안내 보기</a>
          <a class="btn" href="${loginUrl}">구독자 로그인</a>
          <div class="mini">이 안내는 빅데이터센터 진입 시에만 표시됩니다.</div>
        </div>
      </div>
    </div>
  </div>
...
async function handlePostsList(request, env){
  ...
  const bigCats = ['strong','accum','suspicious'];
  // 비구독자(비로그인)는 빅데이터 목록을 볼 수 없습니다.
  // 단, 레거시 지원: sample=1 요청은 is_sample=true 항목만 공개합니다.
  if(!payload && bigCats.includes(category) && !sampleOnly){
    return jsonResp({ok:true, updated_at:new Date().toISOString(), count:0, items:[], gated:true}, 200);
  }
...
async function handlePostsGet(request, env){
  ...
  const bigCats = ['strong','accum','suspicious'];
  if(meta && bigCats.includes(String(meta.category||''))){
    const isSampleLegacy = !!meta.is_sample;
    if(!payload && !isSampleLegacy){
      return jsonResp({ok:false, error:'SUBSCRIBER_ONLY'}, 200);
    }
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    // ==============================
    // Subscriber-only gate: Bigdata pages (non-API)
    // ==============================
    if (isBigdataPath(url.pathname)) {
      const payload = await requireAuth(request, env, url.origin);
      if (!payload) {
        const html = buildMembershipGateHtml(url);
