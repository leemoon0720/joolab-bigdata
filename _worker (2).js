...
function isPublicPath(pathname) {
  ...
  if (pathname.startsWith('/chartgame/')) return true;
...
async function handlePostsList(request, env){
  ...
  // Bigdata is members-only; sample=1 is public
  const bigCats = ['strong','accum','suspicious'];
  if(bigCats.includes(category) && !sampleOnly){
...
async function handlePostsLatest(request, env){
  ...
  // Bigdata latest is members-only
  if(scope === 'bigdata'){
    const payload = await requireAuth(request, env, url.origin);
...
async function handlePostsGet(request, env){
  ...
  // Bigdata posts are members-only; sample posts are public
  const bigCats = ['strong','accum','suspicious'];
  const cat = meta ? String(meta.category||'').trim() : '';
  const isSample = !!(meta && meta.is_sample);
  if(meta && bigCats.includes(cat) && !isSample){
    const payload = await requireAuth(request, env, url.origin);
    if(!payload) return jsonResp({ok:false, error:'FORBIDDEN'}, 200);
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    // ==============================
    // Bigdata gate: redirect to home for logged-out users (do not show login first)
    if (isBigdataPath(url.pathname)) {
      const payload = await requireAuth(request, env, url.origin);
      if (!payload) {
        const toHome = new URL('/', url.origin);
        return Response.redirect(toHome.toString(), 302);
      }
    }
    }
