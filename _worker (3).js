...
const BIGDATA_CATEGORIES = new Set(['strong','accum','suspicious']);
const PUBLIC_SAMPLE_LIMIT = 1;
...
async function handlePostsList(request, env, baseUrl, authPayload){
  ...
  const isBigdata = BIGDATA_CATEGORIES.has(category);
  const isLoggedIn = Boolean(authPayload && authPayload.email);
  const effectiveLimit = (isBigdata && !isLoggedIn) ? Math.min(limit, PUBLIC_SAMPLE_LIMIT) : limit;

  const allowed = ['strong','accum','suspicious','perf','meme'];
  if(!allowed.includes(category)) return jsonResp({ok:false, error:'BAD_CATEGORY', items:[]}, 200);

  if(isBigdata && !isLoggedIn && !sampleOnly){
  ...
  if(region && region !== 'ALL'){
    ...
    items = await _listMetaByPrefix(env, prefix, effectiveLimit);
  }else{
    const a = await _listMetaByPrefix(env, `posts/meta/${category}/KR/`, effectiveLimit);
    const b = await _listMetaByPrefix(env, `posts/meta/${category}/US/`, effectiveLimit);
    items = a.concat(b);
  }
...
async function handlePostsLatest(request, env, baseUrl, authPayload){
  if(!env || !env.JLAB_KV) return jsonResp({ok:false, error:'KV_MISSING'}, 200);
  const url = new URL(request.url);
  const scope = String(url.searchParams.get('scope')||'bigdata').trim();
  const isLoggedIn = Boolean(authPayload && authPayload.email);
  if(scope === 'bigdata' && !isLoggedIn){
...
async function handlePostsGet(request, env, baseUrl, authPayload){
  if(!env || !env.JLAB_KV) return jsonResp({ok:false, error:'KV_MISSING'}, 200);
  const url = new URL(request.url);
  ...
  const isLoggedIn = Boolean(authPayload && authPayload.email);
  const cat = meta && meta.category ? String(meta.category).trim() : '';
  const isBigdata = BIGDATA_CATEGORIES.has(cat);
  const isSample = Boolean(meta && meta.is_sample);
  if(isBigdata && !isLoggedIn && !isSample){
    return jsonResp({ok:false, error:'AUTH_REQUIRED'}, 200);
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    // authPayload는 필요한 요청에서만 계산합니다(빅데이터 페이지 + posts read API).
    let authPayload = null;
    const needAuth = isBigdataPath(url.pathname) || url.pathname === '/api/posts/list' || url.pathname === '/api/posts/get' || url.pathname === '/api/posts/latest';
    if (needAuth) {
      authPayload = await requireAuth(request, env, url.origin);
    }
    ...
    if (url.pathname === '/api/posts/list') {
      return await handlePostsList(request, env, url.origin, authPayload);
    }
    if (url.pathname === '/api/posts/get') {
      return await handlePostsGet(request, env, url.origin, authPayload);
    }
    if (url.pathname === '/api/posts/latest') {
      return await handlePostsLatest(request, env, url.origin, authPayload);
    }
    ...
    // ==============================
    // Targeted login gate
    // - Home/공지/뉴스/성과/리포트/샘플 등은 비로그인 공개
    // - 빅데이터(/data, /strong, /accum, /suspicious)만 로그인 필요
    // ==============================
    if (isBigdataPath(url.pathname) && !authPayload) {
