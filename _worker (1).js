...
function isPublicPath(pathname) {
  // 공개 허용(로그인 없이 접근)
  // 요구사항(수정):
  // - 홈(/) 포함 '나머지 전체'는 비로그인에서도 공개
  // - 빅데이터(강한/매집/수상해 등)만 비로그인에서는 '아예 접근 불가'
  // - 샘플자료실(/sample)은 비로그인에서도 공개
  ...
  if (pathname === '/notice') return true;
  if (pathname.startsWith('/news/')) return true;
  if (pathname === '/news') return true;

  // bigdata + report viewer
  // NOTE: /data, /strong, /accum, /suspicious 는 '빅데이터'로 분류 (비로그인 차단)
  //       단, /post 는 성과/유머/샘플 게시글 렌더링에도 쓰이므로 공개 유지하고,
  //       실제 접근 통제는 /api/posts/get 에서 처리합니다.
  if (pathname.startsWith('/post/')) return true;
  if (pathname === '/post') return true;
  if (pathname.startsWith('/performance/')) return true;
  if (pathname === '/performance') return true;

  // sample 자료실은 공개
  if (pathname.startsWith('/sample/')) return true;
  if (pathname === '/sample') return true;

  // other public utilities/pages
  if (pathname.startsWith('/ticker/')) return true;
  if (pathname === '/ticker') return true;
  if (pathname.startsWith('/tools/')) return true;
  if (pathname === '/tools') return true;
  if (pathname.startsWith('/life/')) return true;
  if (pathname === '/life') return true;
  if (pathname.startsWith('/mini/')) return true;
  if (pathname === '/mini') return true;
...
export default {
  async fetch(request, env, ctx) {
    ...
    const isBigdataPage = (p) => (
      p === '/data' || p.startsWith('/data/') ||
      p === '/strong' || p.startsWith('/strong/') ||
      p === '/accum' || p.startsWith('/accum/') ||
      p === '/suspicious' || p.startsWith('/suspicious/')
    );
        ...
        try{
          ...
          if (id) {
            ...
            if (metaStr) {
              ...
              if (BIGDATA_CATS.includes(cat) && !isSample) {
                return jsonResp({ok:false, error:'FORBIDDEN'}, 200);
              }
            }
          }
        }catch(e){}
    ...
    if (url.pathname === '/api/posts/latest') {
      // 비로그인: scope=bigdata 최신글 조회 차단
      try{
        const u = new URL(request.url);
        const scope = String(u.searchParams.get('scope')||'bigdata').trim();
        if (scope === 'bigdata') {
          const payload = await requireAuth(request, env, url.origin);
          if (!payload) return jsonResp({ok:false, error:'FORBIDDEN'}, 200);
        }
      }catch(e){}
    ...
    if (!isPublicPath(url.pathname)) {
      const payload = await requireAuth(request, env, url.origin);
      if (!payload) {
        // 비로그인에서 빅데이터 페이지는 '아예 안보이게' 홈으로 돌립니다.
        if (isBigdataPage(url.pathname)) {
          return Response.redirect(new URL('/', url.origin).toString(), 302);
        }
        // 그 외 비공개 페이지는 account로 이동.
